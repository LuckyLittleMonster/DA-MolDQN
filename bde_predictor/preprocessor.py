...
def bond_featurizer(bond, flipped=False):
    """Bond feature string hash for BDE-db2 (preprocess_inputs_cfc.py).

    Format: "AtomA-AtomB BondType [Ring]"
...
def bond_featurizer_alfabet(bond, flipped=False):
    """Bond feature string hash for ALFABET.

    Format: "AtomA-AtomB (BondType, IsConjugated) [Ring]"
    Example: "C-O (rdkit.Chem.rdchem.BondType.SINGLE, False)"
    """
    if not flipped:
        atoms = "{}-{}".format(
            bond.GetBeginAtom().GetSymbol(),
...
class BDEPreprocessor:
    ...
    def __init__(self, preprocessor_json_path: str):
        ...
        # Auto-detect bond featurizer format from vocab keys
        bond_keys = config['bond_tokenizer']['_data']
        if any('BondType' in k for k in bond_keys if k != 'unk'):
            self._bond_featurizer = bond_featurizer_alfabet
        else:
    ...
    def process_smiles(self, smiles: str) -> dict:
        ...
        for bond in bonds:
            ...
            bond_tokens.append(self.bond_tokenizer(
                self._bond_featurizer(bond, flipped=False)))
            connectivity.append([begin, end])
            bond_indices_list.append(idx)

            # Reverse edge: end -> begin
            bond_tokens.append(self.bond_tokenizer(
                self._bond_featurizer(bond, flipped=True)))
