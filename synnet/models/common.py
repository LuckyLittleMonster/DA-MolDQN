...
# Fix for PyTorch 2.x weights_only=True default
...
def _load_mlp_from_iclr_ckpt(ckpt_file: str):
    ...
    # Fallback if parent dir name is generic (e.g. "ref")
    if model not in ["act", "rt1", "rxn", "rt2"]:
        stem = Path(ckpt_file).stem
        if "act" in stem: model = "act"
        elif "rt1" in stem: model = "rt1"
        elif "rxn" in stem: model = "rxn"
        elif "rt2" in stem: model = "rt2"

    
    # Check if we are loading reference models (small)
    is_ref = "ref" in str(Path(ckpt_file).parent)
    ...
    if model == "act":
        model = MLP.load_from_checkpoint(
            ...
            input_dim=3 * 4096,
            output_dim=4,
            hidden_dim=100 if is_ref else 1000,
            num_layers=3 if is_ref else 5,
            task="classification",
            dropout=0.5,
        )
    elif model == "rt1":
        model = MLP.load_from_checkpoint(
            ckpt_file,
            input_dim=3 * 4096,
            output_dim=300 if is_ref else 256,
            hidden_dim=100 if is_ref else 1200,
            num_layers=3 if is_ref else 5,
            task="regression",
            dropout=0.5,
        )
        ...
        model = MLP.load_from_checkpoint(
            ckpt_file,
            input_dim=4 * 4096,
            output_dim=3 if is_ref else 91,
            hidden_dim=100 if is_ref else 3000,
            num_layers=5, # rxn is deep even in test
            task="classification",
            dropout=0.5,
        )
    elif model == "rt2":
        model = MLP.load_from_checkpoint(
            ckpt_file,
            input_dim=4 * 4096 + (3 if is_ref else 91),
            output_dim=300 if is_ref else 256,
            hidden_dim=100 if is_ref else 3000,
            num_layers=3 if is_ref else 5,
            task="regression",
