...
def update_route(
    ...
    # Save old block info for rollback on failure
    old_block_idx = step.block_idx
    old_block_smi = step.block_smi
    ...
    # Restore on failure and re-execute to fix intermediate_smi
    step.block_idx = old_block_idx
    step.block_smi = old_block_smi
    route.forward_execute(position, tp)
    return None
...
def extend_route(
    ...
    """Append a new bi-molecular reaction step at the end of the route.

    Args:
        route: Route to extend (modified in-place).
        bi_rxn_idx: Index into tp.bi_reactions.
        block_idx: Building block index.
        tp: TemplateReactionPredictor instance.

    Returns:
        New final product SMILES, or None if the reaction fails.
    """
    bi_rxn = tp.bi_reactions[bi_rxn_idx]
    blk_smi, blk_mol = tp.bb_library[block_idx]
    final_mol = Chem.MolFromSmiles(route.final_product_smi)
    if final_mol is None:
        return None

    products = bi_rxn.forward_smiles(final_mol, blk_mol)
    if not products:
        return None
    ...
    new_step = RouteStep(
        ...
        block_idx=block_idx,
        block_smi=blk_smi,
        intermediate_smi=products[0],
        is_uni=False,
    )
    route.steps.append(new_step)
    route.final_product_smi = products[0]
    return products[0]


def truncate_route(route: SynthesisRoute) -> str | None:
    """Remove the last step of a route.

    Args:
        route: Route to truncate (modified in-place).

    Returns:
        New final product SMILES, or None if route is too short (< 2 steps).
    """
    if len(route.steps) < 2:
        return None
    route.steps.pop()
    route.final_product_smi = route.steps[-1].intermediate_smi
    return route.final_product_smi
