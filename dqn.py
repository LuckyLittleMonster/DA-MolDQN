...
def make_transformer_model(d_atom, N=2, d_model=128, h=8, dropout=0.1, 
...
class GraphTransformer(nn.Module):
    ...
    def _training_step(self, src, src_mask, adj_matrix, distances_matrix, edges_att, y_real):
    ...
    def testing_step(self, src, src_mask, adj_matrix, distances_matrix, edges_att, y_real):
        with torch.no_grad():
            y_pred = self.forward(src, src_mask, adj_matrix, distances_matrix, edges_att)
            loss = self.loss(y_pred, y_real)
        return loss, y_pred
